...
router.post("/create", (req, res) => {
    ...
    const { name, email, age, mobile, price, add, desc } = req.body;

    if (!name || !email || !age || !mobile || !price || !add || !desc) {
    ...
    try {
        conn.query("SELECT * FROM users WHERE email = ?", email, (err, result) => {
            if (result.length) {
                ...
                conn.query("INSERT INTO users SET ?", { name, email, age, mobile, price, add, desc }, (err, result) => {
                    if (err) {
                        console.log("err" + err);
                    } else {
